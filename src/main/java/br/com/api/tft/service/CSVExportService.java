...
public class CSVExportService {
    ...
    public String exportTraitDataToCSV(List<MatchDTO> data, String playerName) throws IOException {
        ...
        try (FileWriter writer = new FileWriter(filename)) {
            ...
            for (MatchDTO matchDTO : data) {
                if (matchDTO.info().queue_id() == 1100) {
                    for (ParticipantDTO p : matchDTO.info().participants()) {
                        for (TraitDto t : p.traits()) {
                            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm").withZone(ZoneId.systemDefault());
                            Instant instant = Instant.ofEpochMilli(matchDTO.info().game_datetime());
                            String gameDatetime = formatter.format(instant);
                            writer.write(String.format(
                                    Locale.US,
                                    "%s,%s,%f,%s,%s,%d,%d,%s,%s,%d," +
                                    "%d,%d,%d,%f,%d,%s,%d,%d,%d,%d\n",
                                    matchDTO.metadata().match_id(), gameDatetime, matchDTO.info().game_length(), matchDTO.info().tft_game_type(), matchDTO.info().tft_set_core_name(), matchDTO.info().tft_set_number(), matchDTO.info().mapId(), p.riotIdGameName() + "#" + p.riotIdTagline(), p.win(), p.level(),
    ...
    public String exportUnitDataToCSV(List<MatchDTO> data, String playerName) throws IOException {
        ...
        try (FileWriter writer = new FileWriter(filename)) {
            writer.write("match_id,date_time,game_duration,game_type,set_name,set_number,map_id,player_nick,win,level," +
                    "gold_left,placement,players_eliminated,time_eliminated,damage_to_players,character,item,rarity,tier\n"
            );
                    ...
                    for (UnitDto u : p.units()) {
                        for (String item : u.itemNames()) {
                            ...
                            writer.write(String.format(
                                    ...
                                    "%d,%d,%d,%f,%d,%s,%s,%d,%d\n",
                                    matchDTO.metadata().match_id(), gameDatetime, matchDTO.info().game_length(), matchDTO.info().tft_game_type(), matchDTO.info().tft_set_core_name(), matchDTO.info().tft_set_number(), matchDTO.info().mapId(), p.riotIdGameName() + "#" + p.riotIdTagline(), p.win(), p.level(),
                                    p.gold_left(), p.placement(), p.players_eliminated(), p.time_eliminated(), p.total_damage_to_players(), u.character_id(), item, u.rarity(), u.tier()
                            ));
                        }
                    }
                }
